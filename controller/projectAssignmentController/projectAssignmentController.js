  ...
  try {
    ...
    if (allocatedAmount <= 0) {
      return res.status(400).json({
        success: false,
        message:
          "Allocated amount must be greater than 0. Please provide valid estimatedHours, rate, estimatedConsumables, or estimatedMaterials.",
      });
    }
    ...
    const totalAllocated =
      (await ProjectAssignment.sum("allocatedAmount", {
        where: {
          projectId,
          isActive: true,
        },
    ...
    const remainingBudget =
      parseFloat(project.budget) - parseFloat(totalAllocated);
    ...
    project.allocatedAmount =
      parseFloat(project.allocatedAmount || 0) + parseFloat(allocatedAmount);
    await project.save();
...
const handleUpdateAssignmentRole = async (req, res) => {
  try {
    ...
    const hours =
      parseFloat(
        actualHours !== undefined ? actualHours : assignment.actualHours
      ) || 0;
    const rate = parseFloat(assignment.rate) || 0;
    const consumables =
      parseFloat(
        actualConsumables !== undefined
          ? actualConsumables
          : assignment.actualConsumables
      ) || 0;
    const materials =
      parseFloat(
        actualMaterials !== undefined
          ? actualMaterials
          : assignment.actualMaterials
      ) || 0;
    ...
    res.status(200).json({
      success: true,
      message:
        "Assignment updated successfully with recalculated allocation amount",
...
const handleSubmitWork = async (req, res) => {
  try {
    ...
    const { actualHours, actualConsumables, actualMaterials } = req.body;
    ...
    if (
      ...
      assignment.workStatus === "approved"
    ) {
      return res.status(400).json({
        success: false,
        message: "Work has already been submitted or approved.",
      });
    }

    // Update work status and submission time
    assignment.workStatus = "submitted";
    assignment.workSubmittedAt = new Date();
    
    // Update actual values
    assignment.actualHours = actualHours || 0;
    assignment.actualConsumables = actualConsumables || 0;
    assignment.actualMaterials = actualMaterials || 0;
    
    // Calculate actual amount using the assignment's rate
    const actualAmount = calculateAllocatedAmount(
      actualHours || 0,
      assignment.rate,
      actualConsumables || 0,
      actualMaterials || 0
    ...
    await Notification.create({
      ...
      metadata: {
        ...
        actualConsumables: actualConsumables || 0,
        actualMaterials: actualMaterials || 0,
  ...
  try {
    ...
    await Notification.create({
      ...
      message: isActive
        ...
        : `Your assignment for project "${
            assignment.project.name
          }" has been disabled. Reason: ${reason || "Not specified"}`,
...
const handleGetOngoingProjects = async (req, res) => {
  try {
    ...
    const notStarted = ongoingAssignments.filter(
      (a) => a.workStatus === "not_started"
    ).length;
    const inProgress = ongoingAssignments.filter(
      (a) => a.workStatus === "in_progress"
    ).length;

    // Calculate total allocated amount
    const totalAllocated = ongoingAssignments.reduce(
...
const handleGetCompletedProjects = async (req, res) => {
  try {
    ...
    // Use actualAmount (manager-approved) if available, otherwise fall back to allocatedAmount
    const totalEarned = completedAssignments.reduce(
      (sum, a) => sum + (parseFloat(a.actualAmount || a.allocatedAmount) || 0),
      0
    );
    ...
    completedAssignments.forEach((assignment) => {
      if (assignment.workVerifiedAt) {
        ...
        // Use actualAmount (manager-approved) if available, otherwise fall back to allocatedAmount
        completionsByMonth[monthKey].earned +=
          parseFloat(assignment.actualAmount || assignment.allocatedAmount) || 0;
      }
    });
    ...
    res.json({
      ...
      summary: {
        ...
        totalEarned,
        averagePerProject:
          totalCompleted > 0 ? (totalEarned / totalCompleted).toFixed(2) : 0,
      },
